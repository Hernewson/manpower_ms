...
// class Date_Conversion extends MX_Controller{

function submit()
{
	echo $date=$_POST['date'];
	$date_array=  explode('-', $date);
	$english_date=$this->nep_to_eng($date_array[0], $date_array[1], $date_array[2]);
	var_dump($english_date);
	var_dump($english_date['year'].'-'.$english_date['month'].'-'.$english_date['date']); die;
...
private $bs = array(
	...
	99=>array(2099, 31, 31, 32, 31, 31, 31, 30, 29, 29, 30, 30, 30)
	);
...
{
	$array_nep=  $this->input->post('nep',TRUE);
	$data['nep']= implode(",",(array)$array_nep);
	return $data;
}
...
private function is_range_nep($yy, $mm, $dd){		
	if($yy<2000 || $yy>2099){
		$this->debug_info="Supported only between 2000-2099";
		return false;
	}
